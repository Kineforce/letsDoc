    ...
    $.ajax({
      ...
      success: (response) => {
        if (response.count == 0) {
          ...
          $(".as-content").append('<span class="as_modal_open"><a href="#as_modal" rel="modal:open">Adicionar servidor</a></span>');
        ...
        $(".as-content").append('<span class="as_modal_open"><a href="#as_modal_cria_server" rel="modal:open">Adicionar servidor</a></span>');
  ...
  html += '<div class="as-card">'
    html += '<div class="as-content-card">';
      ...
      html += '<div class="as-dropdown">'
        html += '<i class="fas fa-caret-square-down" onclick="mostraSubItemsServidor(event)"></i>'
      html += '</div>'
      ...
      html += '<div class="as-exclusao">';
        html += '<i class="fa fa-trash as-excluir" aria-hidden="true" onclick="deletaDadosServidor(event)"></i>';
      html += "</div>";
      html += '<div class="as-update">';
        html += '<a href="#as_modal_update_server" id="update_server" rel="modal:open" onclick="openModalUpdate(event)"><i class="fa fa-wrench as-update"></i></a>'
      html += "</div>";
    html += "</div>";
...
function retornaSubCardHtml(id_item, status_item, nome_item, descricao) {
  ...
  html += '<div class="as-subcard-item">'   
    ...
    html += '<div class="as-subcard-exclusao">';
        html += '<i class="fa fa-trash as-excluir_subitem" aria-hidden="true" onclick="deletaDadosSubItemServidor(event)"></i>';
      html += "</div>";
      html += '<div class="as-subcard-update">';
        html += '<a href="#as_modal_update_server_subitem" id="update_server" rel="modal:open" onclick="openModalUpdateSubItem(event)"><i class="fa fa-wrench as-update"></i></a>'
      html += "</div>";
  html += '</div>'
...
function mostraSubItemsServidor(event){
  ...
  let curr_sub_card = current_dropdown_btn.closest('.as-card').find('.as-content-server-card');
  ...
  $.ajax({
    ...
    success: (resp)=>{
      ...
      $(curr_sub_card).append('<span class="as_modal_open"><a href="#as_modal_cria_server_subitem" rel="modal:open" id="as_subitem_servidor_create_btn" onclick="openModalCreateSubItem(event)">Adicionar item</a></span>');

    },
    error: ()=>{
      Swal.fire(
        'Algo de errado ocorreu na busca do registro!',
        '',
        'error'
      )
    }
